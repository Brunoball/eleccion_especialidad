...
function ok($data=[], $code=200){ http_response_code($code); echo json_encode(['exito'=>true] + $data); exit; }
function fail($msg, $code=200){ http_response_code($code); echo json_encode(['exito'=>false,'mensaje'=>$msg]); exit; }
...
$allowedOrder = [
  'alumno'=>'a.alumno','dni'=>'a.dni','promedio'=>'a.promedio','inasistencias'=>'a.inasistencias',
  'matprev'=>'a.matprev','matpend'=>'a.matpend','repite'=>'a.repite','sanciones'=>'a.sanciones',
  'rendir'=>'a.rendir','id_alumno'=>'a.id_alumno'
];
...
$params = [];
if ($q !== '') {
  if (ctype_digit($q)) { $where[] = 'a.dni = :dni'; $params[':dni'] = $q; }
  else { $where[] = 'a.alumno LIKE :alumno'; $params[':alumno'] = '%'.$q.'%'; }
}
$whereSql = $where ? 'WHERE '.implode(' AND ', $where) : '';

/* total */
$sqlCount = "SELECT COUNT(*) FROM eleccion_especialidad.alumnos a $whereSql";
$st = $pdo->prepare($sqlCount); $st->execute($params);
$total = (int)$st->fetchColumn();

/* datos + JOIN a elección y especialidad */
$sql = "
  SELECT
  ...
  FROM eleccion_especialidad.alumnos a
  LEFT JOIN eleccion_especialidad.eleccion e ON e.id_alumno = a.id_alumno
  LEFT JOIN eleccion_especialidad.especialidad s ON s.id_especialidad = e.id_especialidad
  $whereSql
